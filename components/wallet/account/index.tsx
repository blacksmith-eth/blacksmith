      ...
      <section className="flex flex-col gap-2 font-mono">
        <Listbox value={address} onChange={setAddress}>
          ...
          <div className="relative text-right select-none text-sm">
            <Listbox.Button className="text-left border border-black h-full w-full px-2 py-1">
              {formatAddress(address)}
              {balance &&
                ` | ${formatValue(balance.formatted)} ${balance.symbol}`}
            </Listbox.Button>
            <Listbox.Options className="text-left w-full bg-white border border-black absolute mt-1 right-0 focus:outline-none">
